...
resource "aws_eks_cluster" "main" {
  ...
  version  = "1.28" # Stable version with good compatibility
...
# ---------------------------
# EKS Node Group (1 t4g.nano spot)
# ---------------------------
...
# Use the existing ALB controller policy from IAM module
resource "aws_iam_role_policy_attachment" "aws_load_balancer_controller" {
  policy_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:policy/${var.project_name}-alb-controller-policy"
...
output "oidc_provider_url" {
  value = "https://${replace(aws_eks_cluster.main.identity[0].oidc[0].issuer, "https://", "")}"
}
